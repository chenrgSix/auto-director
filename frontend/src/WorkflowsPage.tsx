...
import type { Asset, Binding, Capabilities, Job, Parameter, Settings, Value, Workflow } from './types';
...
export default function WorkflowsPage({ notify }: { notify: Notify }) {
  ...
  async function importFile() {
    setBusy(true);
    try {
      const result = await api<Workflow>('/workflows/import', 'POST', { name, type, workflow: graph });
  ...
  return <><div className="section-heading"><div><span className="eyebrow">YOUR RENDER ENGINES</span><h1>工作流</h1><p>连接你熟悉的 ComfyUI 工作流，自由选择每一幅画面的生成方式。</p></div><button className="primary" onClick={() => setImporting(!importing)}><Plus size={16} />导入工作流</button></div>
    ...
    {importing && <section className="panel import-panel"><div className="panel-title"><FileJson size={17} />导入 ComfyUI API Format JSON</div><div className="fields two"><div className="field"><label htmlFor="import-name">工作流名称</label><input id="import-name" value={name} maxLength={120} onChange={event => setName(event.target.value)} /></div><div className="field"><label htmlFor="import-type">输出类型</label><select id="import-type" value={type} onChange={event => setType(event.target.value as 'image' | 'video')}><option value="image">图像</option><option value="video">视频（首帧 / 首尾帧）</option></select></div></div><label className="upload-zone"><Upload size={23} /><span>{filename || '选择 API JSON 文件'}</span><small>ComfyUI → Save (API Format) · 最大 2 MiB</small><input type="file" accept="application/json,.json" onChange={event => void selectFile(event)} /></label><div className="actions"><button onClick={() => setImporting(false)}>取消</button><button className="primary" disabled={!graph || !name.trim() || busy} onClick={() => void importFile()}>分析并导入</button></div></section>}
    {!resource.data ? <Loading /> : <div className="workflow-grid"><aside className="workflow-list">{resource.data.map(item => <button className={`workflow-card ${current?.id === item.id ? 'selected' : ''}`} key={item.id} onClick={() => setSelected(item.id)}><span className="workflow-icon">{item.type === 'image' ? <Image size={21} /> : <Video size={21} />}</span><div><strong>{item.name}</strong><small>{item.type === 'image' ? 'TEXT TO IMAGE' : 'FRAME TO VIDEO'}</small><span className="workflow-state">{config.data?.[`default_${item.type}`] === item.id && <span><Star size={11} />默认</span>}{item.validation?.valid ? <span className="success-text"><Check size={11} />校验通过</span> : <span>{item.validation ? '依赖待处理' : '未校验'}</span>}</span></div></button>)}<div className="workflow-help"><Layers3 size={20} /><p>模型权重由 ComfyUI 管理。你可以更换默认工作流，也可以独立编辑每个参数。</p></div></aside>{current && <WorkflowEditor key={current.id} workflow={current} isDefault={config.data?.[`default_${current.type}`] === current.id} notify={notify} refresh={() => { resource.refresh(); config.refresh(); }} />}</div>}
  </>;
}

function ParameterInput({ parameter, value, onChange }: { parameter: Parameter; value: Value; onChange: (value: Value) => void }) {
  if (parameter.type === 'boolean') return <input aria-label={parameter.key} type="checkbox" checked={Boolean(value)} onChange={event => onChange(event.target.checked)} />;
  if (parameter.enum?.length) return <select aria-label={parameter.key} value={String(value)} onChange={event => { const selected = parameter.enum!.find(item => String(item) === event.target.value); if (selected !== undefined) onChange(selected); }}>{!parameter.enum.includes(value) && <option value={String(value)}>{String(value)}（当前不可用）</option>}{parameter.enum.map(item => <option key={String(item)} value={String(item)}>{String(item)}</option>)}</select>;
  if (parameter.type === 'textarea' || parameter.field === 'text') return <textarea aria-label={parameter.key} value={String(value)} onChange={event => onChange(event.target.value)} rows={3} />;
  const numeric = ['integer', 'number'].includes(parameter.type);
  return <input aria-label={parameter.key} type={numeric ? 'number' : 'text'} value={String(value)} min={parameter.min ?? undefined} max={parameter.max ?? undefined} step={parameter.type === 'integer' ? 1 : parameter.step ?? 'any'} onChange={event => onChange(numeric ? Number(event.target.value) : event.target.value)} />;
}

function WorkflowEditor({ workflow, isDefault, notify, refresh }: { workflow: Workflow; isDefault: boolean; notify: Notify; refresh: () => void }) {
  ...
  const roles = [...new Set(['prompt', 'negative', 'width', 'height', 'seed', ...(workflow.type === 'video' ? ['start_frame', 'end_frame', 'duration', 'fps', 'reference_video'] : ['reference_image', 'style_reference']), ...Object.keys(bindings), ...extraRoles])];
  ...
  async function save() { return perform(`/workflows/${workflow.id}`, '工作流已保存，请重新校验', { name, bindings, outputs, capabilities: caps, parameter_values: parameters }, 'PATCH'); }
  ...
  async function testRun() {
    setBusy(true);
    try {
      await api(`/workflows/${workflow.id}`, 'PATCH', { name, bindings, outputs, capabilities: caps, parameter_values: parameters });
