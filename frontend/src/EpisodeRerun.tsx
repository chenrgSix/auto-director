...
export function EpisodeRerun({ episode, shot, locked, busy, setBusy, refresh, notify }: Props) {
  ...
  const affected = episode.shots.filter(s => {
    ...
    if (sequence) return !!episode.sequence_groups?.some(group => group.shot_ids.includes(s.id) && selected.some(item => group.shot_ids.includes(item.id)));
  ...
  async function submit() {
    setBusy(true);
    try {
      await api(`/episodes/${episode.id}/rerun`, 'POST', { expected_version: version, scope, new_seed: newSeed, ...(sequence || staticFrames === 'keep' ? {} : { allow_static_end_frame: staticFrames === 'allow' }), ...(target === 'all' ? {} : { shot_ids: [target] }) });
  ...
  return <section className="panel">
    ...
    {open && <div className="rerun-form">
      ...
      <label>生成内容<select value={scope} disabled={locked || busy} onChange={e => setScope(e.target.value)}><option value="video">{sequence ? '重新生成连续组视频（沿用参考图）' : '仅重新生成视频（沿用已有关键帧）'}</option>{!sequence && <option value="keyframes">重新生成关键帧及视频</option>}{!episode.creation_source && <option value="prompts">{sequence ? '重写连续组提示词并生成视频' : '重新生成提示词、关键帧及视频'}</option>}</select></label>
      ...
      {!sequence && <label>首尾帧变化要求<select value={staticFrames} disabled={locked || busy} onChange={e => setStaticFrames(e.target.value)}><option value="keep">沿用各镜头设定</option><option value="change">要求首尾帧有变化</option><option value="allow">允许静止首尾帧（有意定格）</option></select><small className="muted">仅影响所选镜头的首尾帧视频检测；图生视频不检查尾帧。</small></label>}
      <div className="notice">将重跑 {selected.length} 镜，连同连续性依赖共影响 {affected.length} 镜，并重新合成整片。{scope === 'prompts' ? '分镜顺序和时长保持，所选镜头提示词重新编写。' : '剧本与提示词保持。'}旧视频不会覆盖或删除，新任务失败也能查看旧版。{sequence ? `同组片段共用声画上下文，将整组重跑：${affected.map(s => `第 ${s.index + 1} 段`).join('、')}。` : '依赖前镜尾帧的后续镜头会重做关键帧，缺失素材会自动补齐。'}</div>
