...
export function WorkflowEditor({ workflow, isDefault, notify, refresh, maxShotSeconds, onDirtyChange }: { workflow: Workflow; maxShotSeconds?: number; isDefault: boolean; notify: Notify; refresh: () => void; onDirtyChange: (dirty: boolean) => void }) {
  ...
  return <form className="workflow-editor" ref={form} onSubmit={event => event.preventDefault()}><fieldset disabled={busy} className="workflow-fieldset">
    ...
    <section className="panel workflow-pane" role="tabpanel" id="pane-bindings" aria-labelledby="tab-bindings" data-pane="bindings" hidden={pane !== 'bindings'}>
      ...
      {workflow.media_type === 'video' && <div className="field"><label htmlFor="audio-prompt-format">原生声音提示词</label><select id="audio-prompt-format" value={caps.audio_prompt_format ?? 'none'} onChange={event => setCaps({...caps, audio_prompt_format: event.target.value as 'none' | 'minimax_h3'})}><option value="none">普通视频提示词</option><option value="minimax_h3">MiniMax H3 · 对白 / 旁白 / 环境声</option></select><small>H3 需已连接音频解码和视频音轨输出。新分镜会按剧本编写声音；已有提示词保持，需选择重新生成提示词才会更新。</small></div>}
    ...
    <section className="panel workflow-pane" role="tabpanel" id="pane-checks" aria-labelledby="tab-checks" data-pane="checks" hidden={pane !== 'checks'}>
      ...
      <div className="test-panel capabilities" data-test-inputs><h3>试跑验证</h3><p className="muted">以下操作会提交真实生成任务。先检查配置，再提供测试描述与素材。</p><div className="field"><label htmlFor="test-prompt">测试画面描述</label><textarea id="test-prompt" rows={3} value={testPrompt} onChange={event => setTestPrompt(event.target.value)} /></div>{workflow.media_type === 'video' && <div className="field"><label htmlFor="test-duration">试跑时长（秒，{testFps} FPS）</label><input id="test-duration" type="number" required min={1} max={caps.max_duration} step={0.1} value={testDuration} onChange={event => setTestDuration(Number(event.target.value))} /></div>}{(capability === 'REFERENCE_SEQUENCE_TO_VIDEO' ? ['reference_image_1', 'reference_image_2', 'reference_image_3'] : Object.keys(bindings).filter(isAssetRole)).map(role => <label key={role} className="file-field"><span>{roleLabel(role)} {testAssets[role] && <Check size={14} />}</span><input type="file" accept={role === 'reference_video' ? 'video/*' : role === 'reference_audio' ? 'audio/*' : 'image/png,image/jpeg,image/webp'} onChange={event => void upload(role, event.target.files?.[0])} /></label>)}<button type="button" disabled={!testPrompt.trim() || (capability === 'REFERENCE_SEQUENCE_TO_VIDEO' && !Object.keys(testAssets).length) || !!missing.length || !!unboundAssets.length} onClick={() => void testRun()}><Play size={15} />保存并试跑</button>{jobId && <TestJob id={jobId} kind={workflow.media_type} notify={notify} />}</div>
    </section>
  </fieldset></form>;
}
