...
import { ArrowUpRight, Clapperboard, Film, Layers3, Plus, Settings2, X } from 'lucide-react';
...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="主导航">{menu.map(item => <a key={item.id} href={`#${item.id}`} className={route === item.id || (item.id === 'episodes' && route.startsWith('episode/')) ? 'active' : ''}><item.icon size={18} />{item.label}{item.id === 'create' && <span className="nav-plus">＋</span>}</a>)}</nav>
      ...
      <a href="#setup" className="connection-card"><span className={`dot ${config.error ? 'offline' : config.data?.llm_configured ? 'online' : ''}`} /><div><strong>{config.error ? '后端尚未连接' : config.data?.llm_configured ? '本地工作台已就绪' : '完成首次配置'}</strong><small>{config.error ? '检查本地服务' : 'ComfyUI · 模型服务'}</small></div><ArrowUpRight size={15} /></a>
    </aside>
    <main className="main">
      <header className="topbar"><span>STUDIO <span className="slash">/</span> {route.startsWith('episode/') ? '短片详情' : menu.find(item => item.id === route)?.label ?? '开始创作'}</span><div><span className="local-pill"><i />LOCAL FIRST</span><span className="avatar">AD</span></div></header>
      <div className="page" key={route}>
        {route === 'episodes' ? <EpisodesPage notify={notify} /> : route.startsWith('episode/') ? <EpisodePage id={route.slice(8)} notify={notify} /> : route === 'workflows' ? <WorkflowsPage notify={notify} /> : route === 'setup' ? <SetupPage notify={notify} onChange={config.refresh} /> : <CreatePage notify={notify} />}
      </div>
    </main>
