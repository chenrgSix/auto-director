...
type Report = { valid: boolean; revision: number; constraints_hash: string | null; issues: { code: string; message: string; details?: unknown }[] };
...
function ProjectEditor({ project, notify, refresh, disconnected }: { project: Project; notify: Notify; refresh: () => void; disconnected: boolean }) {
  ...
  return <>
    ...
    <div className="section-heading"><div><span className="eyebrow">CREATION PACKAGE · VERSION {base.revision}</span><h1>{base.title}</h1><p>持续打磨创作包；每次交付都保留一份独立的制作快照。</p></div><div className="actions"><button disabled={busy} onClick={() => download(`creation-${project.id}-v${base.revision}${dirty || jsonText !== undefined ? '-draft' : ''}.json`, jsonText ?? draft, jsonText !== undefined)}><Download size={15} />导出{dirty || jsonText !== undefined ? '当前草稿' : '创作包'}</button><button disabled={busy} onClick={() => void run(async () => { await navigator.clipboard.writeText(jsonText ?? JSON.stringify(draft, null, 2)); notify('当前创作内容已复制'); })}>复制 JSON</button></div></div>
    ...
    <div className="creation-editor-layout"><section className="panel"><div className="panel-title">创作版本</div>{project.history.map(item => <div className="creation-history-row" key={item.revision}><span>版本 {item.revision}<small>{new Date(item.created_at).toLocaleString('zh-CN')}</small></span><button disabled={busy} onClick={() => void run(async () => { download(`creation-${project.id}-v${item.revision}.json`, await api(`${path}/export?revision=${item.revision}`)); })}><Download size={14} />导出此版本</button></div>)}</section>
