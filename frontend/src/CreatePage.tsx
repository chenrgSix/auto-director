...
export default function CreatePage({ notify }: { notify: Notify }) {
  ...
  const [advanced, setAdvanced] = useState({ image_workflow_id: '', video_workflow_id: '', reference_workflow_id: '', memory_mode: 'auto', fps: 16, seed: 42, max_retries: 2 });
  const sequence = workflows.data?.find(item => item.id === (advanced.video_workflow_id || config.data?.default_video))?.capability === 'REFERENCE_SEQUENCE_TO_VIDEO';
  const selectedIds = [sequence ? null : advanced.image_workflow_id || config.data?.default_image, advanced.video_workflow_id || config.data?.default_video, advanced.reference_workflow_id || config.data?.default_capabilities.TEXT_TO_IMAGE];
  ...
  async function submit(event: FormEvent) {
    ...
    setBusy(true);
    try {
      const episode = await api<Episode>('/episodes', 'POST', { idea, target_duration: duration, aspect_ratio: ratio, style, quality, qa_policy: qaPolicy, qa_enabled: qaEnabled, preview_required: true, image_review_required: imageReview, video_workflow_id: advanced.video_workflow_id || null, reference_workflow_id: advanced.reference_workflow_id || null, advanced_mode: advancedMode, ...(advancedMode ? { ...advanced, image_workflow_id: advanced.image_workflow_id || null, video_workflow_id: advanced.video_workflow_id || null, reference_workflow_id: advanced.reference_workflow_id || null, workflow_overrides: activeOverrides } : {}) });
    ...
    <div className="create-grid">
      <form className="create-form" onSubmit={submit}>
        ...
        <div className="panel parameters-panel"><div className="panel-title">定义你的画面 <span className="muted">01 / CREATIVE DIRECTION</span></div>
          <div className="field"><label id="duration-label">短片参考时长</label><div className="duration-options" role="group" aria-labelledby="duration-label">{durationPolicy?.presets.map(value => <button key={value} type="button" aria-pressed={preset === value} className={preset === value ? 'selected' : ''} onClick={() => { setPreset(value); setCustomDuration(''); }}>{value}<span>秒</span></button>)}</div>
            ...
            <small id="duration-hint">{durationPolicy && `支持 ${durationPolicy.min}～${durationPolicy.max} 秒。`}导演按参考时长规划镜头；成片完整拼接模型生成的片段，实际时长可能不同。</small>
          ...
          <div className="field"><label htmlFor="production-workflow">视频制作方式</label><select id="production-workflow" value={advanced.video_workflow_id} onChange={event => setAdvanced({ ...advanced, video_workflow_id: event.target.value })}><option value="">使用默认视频工作流</option>{workflows.data?.filter(item => item.media_type === 'video').map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><small>{sequence ? '多参考连续镜头：确认参考图后，按分组连续生成声画。无需逐镜制作首尾帧，每组最多 8 段。' : '先制作关键帧，再生成各镜头视频。'}</small></div>
          <label className="checkbox"><input type="checkbox" checked={imageReview} onChange={event => setImageReview(event.target.checked)} />生成视频前确认画面（推荐）</label><small>{sequence ? '确认人物、场景和道具参考后直接生成连续视频；可从网页或 Codex 接手。' : '先确认人物、场景和道具参考，再逐镜检查首尾帧；可从网页或 Codex 接手修改。'}</small>
...
<div className="fields two">
            {(['image', 'reference'] as const).filter(type => !sequence || type !== 'image').map(type => <div className="field" key={type}><label htmlFor={`workflow-${type}`}>{type === 'image' ? '关键帧图像工作流' : '初始参考图工作流'}</label><select id={`workflow-${type}`} value={advanced[`${type}_workflow_id`]} onChange={event => setAdvanced({ ...advanced, [`${type}_workflow_id`]: event.target.value })}><option value="">使用默认工作流</option>{workflows.data?.filter(item => type === 'reference' ? item.capability === 'TEXT_TO_IMAGE' : item.media_type === type).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>)}
          ...
          </div>{selectedWorkflows.map(workflow => <ParameterOverrides key={workflow.id} workflow={workflow} values={overrides[workflow.id] ?? {}} notify={notify} onChange={values => setOverrides(current => ({ ...current, [workflow.id]: values }))} />)}</details>}
        </div>
