    ...
    <div className="create-grid">
      <form className="create-form" onSubmit={submit}>
        ...
        <div className="panel parameters-panel"><div className="panel-title">定义你的画面 <span className="muted">01 / CREATIVE DIRECTION</span></div>
          <div className="field"><label>短片时长</label><div className="duration-options">{[5, 10, 15, 20, 30].map(value => <button key={value} type="button" aria-pressed={duration === value} className={duration === value ? 'selected' : ''} onClick={() => setDuration(value)}>{value}<span>秒</span></button>)}</div><small>导演将根据总时长自动规划镜头。</small></div>
        ...
        <div className="generate-row"><p><span className="dot" />自动规划 · 顺序渲染 · 本地保存</p><button className="primary" disabled={busy || !idea.trim() || !workflows.data} type="submit"><Sparkles size={17} />{busy ? '正在创建…' : '开始生成短片'}<ArrowRight size={17} /></button></div>
      </form>
