...
export function EpisodePage({ id, notify }: { id: string; notify: Notify }) {
  ...
  const progress = active && <div className="progress-panel episode-progress"><div><span>{reviewing ? '正在准备分镜预览' : '正在推进创作'}</span><strong>{reviewing ? episode.shots.filter(item => item.prompts).length : completed} / {total || '—'} 镜头{!reviewing && '已完成'}</strong></div>{!reviewing && currentShot && <p>第 {currentShot.index + 1} 镜 · {currentShot.title}：{episode.status === 'QA' && currentShot.video_asset_id ? '视频已生成，正在检查画面；可在镜头预览查看。' : `${STATUS[currentShot.status] ?? currentShot.status}。`}{episode.status === 'QA' && (episode.qa_policy ?? 'strict') === 'advisory' && '画面问题会标记待复核并继续。'}</p>}{!reviewing && reviewShots.length > 0 && <small>{reviewShots.length} 镜已完成，画面待复核，不阻塞后续生成。</small>}<div className="progress-track"><i style={{ width: `${total ? (reviewing ? episode.shots.filter(item => item.prompts).length : completed) / total * 90 : 3}%` }} /></div><div className="episode-progress-actions"><small>进度自动更新，已有视频可随时预览。</small><button className="text-button" onClick={showDiagnostics}>查看生成记录</button></div></div>;
  return <>
    ...
    {episode.final_video_asset_id && <section className="final-panel episode-film" aria-label="成片预览">
      ...
      <div className="film-summary"><span className="eyebrow">FINAL CUT</span><h2>短片已完成</h2><p>{episode.final_duration?.toFixed(2)} 秒 · MP4</p><a className="primary" href={assetUrl(episode.final_video_asset_id, true)} download><Download size={16} />下载成片</a>
    ...
    {reviewing ? episode.status === 'AWAITING_REVIEW' ? <><EpisodePreview key={id} episode={episode} busy={busy} setBusy={setBusy} refresh={() => { resource.refresh(); jobs.refresh(); }} notify={notify} /><button className="text-button" disabled={busy} onClick={() => void action(`/episodes/${id}/preview`, '正在更新分镜预览')}>工作流配置有变化？更新分镜预览</button></> : <Empty title={active ? '导演正在准备分镜和提示词' : '继续准备分镜即可预览'}>预览准备完成后会等待你确认，再开始图片与视频生成。</Empty> : !episode.shots.length ? <Empty title={active ? '导演正在规划故事' : '你的故事，即将成为镜头'}>计划完成后，镜头、首尾帧和视频将在这里出现。</Empty> : <div className="editing-grid episode-editor">
      {shot && <section className="shot-detail" id="episode-shot-detail" aria-label="镜头预览">
        ...
        {shot.enabled && shot.video_asset_id && shot.prompts && <PromptOptimization key={shot.id} episode={episode} shot={shot} locked={settingsLocked} busy={busy} setBusy={setBusy} refresh={() => { resource.refresh(); jobs.refresh(); }} notify={notify} />}
      </section>}
