...
import { PreproductionReview } from './PreproductionReview';
...
export function EpisodePage({ id, notify }: { id: string; notify: Notify }) {
  ...
  const runningGroup = jobs.data?.find(job => job.type === 'SEQUENCE_VIDEO' && job.status === 'RUNNING');
  const active = ACTIVE.has(episode.status);
  const imageReview = episode.status === 'AWAITING_IMAGE_REVIEW';
  ...
  return <>
    ...
    {sequence && <section className="panel sequence-summary" aria-label="连续镜头分组"><div className="panel-title">多参考连续镜头 · {episode.sequence_groups?.length ?? 0} 组</div><p className="muted">参考图确认后直接生成整组声画；每个片段都可预览下载，重跑会包含同组全部片段。</p><div className="sequence-groups">{episode.sequence_groups?.map((group, index) => <button key={group.id} onClick={() => selectReviewShot(group.shot_ids[0])}><strong>第 {index + 1} 组 · {group.shot_ids.length} 段</strong><span>参考 {group.planned_duration.toFixed(2)} 秒</span><Badge status={group.status} /></button>)}</div>{runningGroup?.progress?.segment != null && <p role="status">当前组：第 {runningGroup.progress.segment} / {runningGroup.progress.segment_total} 段 · {runningGroup.progress.phase_label ?? '生成中'}</p>}{episode.sequence_group_error && <ErrorNotice>{episode.sequence_group_error.message}</ErrorNotice>}</section>}
    ...
    {imageReview && <PreproductionReview key={id} episode={episode} refresh={() => { resource.refresh(); jobs.refresh(); }} />}
    {reviewing ? episode.status === 'AWAITING_REVIEW' ? <><EpisodePreview key={id} episode={episode} busy={busy} setBusy={setBusy} refresh={() => { resource.refresh(); jobs.refresh(); }} notify={notify} />{!episode.creation_source && <button className="text-button" disabled={busy} onClick={() => void action(`/episodes/${id}/preview`, '正在更新分镜预览')}>工作流配置有变化？更新分镜预览</button>}</> : episode.shots.length ? <PromptPreparation key={id} episode={episode} /> : <Empty title={active ? '导演正在准备分镜和提示词' : '继续准备分镜即可预览'}>预览准备完成后会等待你确认，再开始图片与视频生成。</Empty> : !episode.shots.length ? <Empty title={active ? '导演正在规划故事' : '你的故事，即将成为镜头'}>计划完成后，镜头、首尾帧和视频将在这里出现。</Empty> : <div className="editing-grid episode-editor">
      {shot && <section className="shot-detail" id="episode-shot-detail" aria-label="镜头预览">
        <div className="panel shot-player">
          ...
          <Media id={shot.video_asset_id || shot.start_frame_asset_id} kind={shot.video_asset_id || !shot.start_frame_asset_id ? 'video' : 'image'} label={shot.video_asset_id ? '镜头预览 / CLIP' : sequence ? '连续片段 · 等待整组视频生成' : '关键帧预览 · 等待视频生成'} />
        ...
        <details className="panel details-panel"><summary>{sequence ? '分镜说明与参考素材' : '分镜说明与首尾帧'}</summary><p className="shot-description">{shot.action}</p><small className="muted">{shot.camera}</small>{sequence ? <div className="reference-grid">{shot.prompts?.visual_continuity?.reference_roles.map(role => <Media key={role} id={episode.references[role]} kind="image" label={role} />)}</div> : <div className="keyframe-grid"><Media id={shot.start_frame_asset_id} kind="image" label="首帧 / START" />{shot.end_frame_asset_id && <Media id={shot.end_frame_asset_id} kind="image" label="尾帧 / END" />}</div>}</details>
        ...
        {!sequence && !episode.creation_source && shot.enabled && shot.video_asset_id && shot.prompts && <PromptOptimization key={shot.id} episode={episode} shot={shot} locked={settingsLocked} busy={busy} setBusy={setBusy} refresh={() => { resource.refresh(); jobs.refresh(); }} notify={notify} />}
      </section>}
      <aside className="panel shot-list">{progress}<div className="panel-title">镜头时间线 <span>{episode.shots.length} SHOTS</span></div>{episode.shots.map((item, index) => <div key={item.id} className={`shot-row ${shot?.id === item.id ? 'selected' : ''} ${!item.enabled ? 'disabled' : ''}`} draggable={!active && !imageReview && !busy && !recovering && !disconnected} onDragStart={() => setDragged(item.id)} onDragOver={event => event.preventDefault()} onDrop={() => { const from = episode.shots.findIndex(item => item.id === dragged); if (from >= 0) move(from, index); setDragged(undefined); }}><button className="shot-select" onClick={() => setSelected(item.id)}><span className="shot-number">{String(index + 1).padStart(2, '0')}</span><div><strong>{item.title}</strong><small>{item.actual_duration != null ? `实际 ${item.actual_duration.toFixed(2)}s` : `参考 ${item.duration.toFixed(2)}s`} · {item.transition_from_previous}</small><Badge status={item.status === 'PASSED' && item.needs_review ? 'NEEDS_REVIEW' : item.status} /></div><GripVertical size={14} /></button><div className="shot-controls"><label><input type="checkbox" checked={item.enabled} disabled={active || imageReview || busy || recovering || disconnected} onChange={event => void updateTimeline(episode.shots.map(s => s.id === item.id ? { ...s, enabled: event.target.checked } : s))} />使用</label><button className="icon-button" aria-label={`上移镜头 ${index + 1}`} disabled={active || imageReview || busy || recovering || disconnected || index === 0} onClick={() => move(index, index - 1)}><ArrowUp size={14} /></button><button className="icon-button" aria-label={`下移镜头 ${index + 1}`} disabled={active || imageReview || busy || recovering || disconnected || index === episode.shots.length - 1} onClick={() => move(index, index + 1)}><ArrowDown size={14} /></button></div></div>)}</aside>
    </div>}
