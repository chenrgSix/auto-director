...
export function EpisodePage({ id, notify }: { id: string; notify: Notify }) {
  ...
  return <>
    ...
    <EpisodeQuality key={id} episode={episode} busy={busy} setBusy={setBusy} locked={settingsLocked} onSaved={() => { resource.refresh(); jobs.refresh(); }} notify={notify} />
    <EpisodeQAPolicy key={id} episode={episode} busy={busy} setBusy={setBusy} locked={settingsLocked} onSaved={() => { resource.refresh(); jobs.refresh(); }} notify={notify} />
    ...
    {active && <div className="progress-panel"><div><span>{reviewing ? '正在准备分镜预览' : '正在推进创作'}</span><strong>{reviewing ? episode.shots.filter(item => item.prompts).length : completed} / {total || '—'} 镜头{!reviewing && '已完成'}</strong></div>{!reviewing && currentShot && <p>第 {currentShot.index + 1} 镜 · {currentShot.title}：{episode.status === 'QA' && currentShot.video_asset_id ? '视频已生成，正在检查画面；可在镜头预览查看。' : `${STATUS[currentShot.status] ?? currentShot.status}。`}{episode.status === 'QA' && (episode.qa_policy ?? 'strict') === 'advisory' && '画面问题会标记待复核并继续。'}</p>}{!reviewing && reviewShots.length > 0 && <small>{reviewShots.length} 镜已完成，画面待复核，不阻塞后续生成。</small>}<div className="progress-track"><i style={{ width: `${total ? (reviewing ? episode.shots.filter(item => item.prompts).length : completed) / total * 90 : 3}%` }} /></div><small>进度会自动更新，可以离开页面，稍后回来查看。</small></div>}
    ...
    {episode.final_video_asset_id && <section className="final-panel"><div><span className="eyebrow">IT'S A WRAP</span><h2>短片已完成。</h2><p>{episode.final_duration?.toFixed(2)} 秒 · MP4</p>{reviewShots.length > 0 && <div className="notice final-review" role="status"><strong>{reviewShots.length} 镜待复核</strong><span>整片已导出，以下镜头有画面问题或尚未完成质检。查看后可使用「选择镜头重跑」，旧视频会保留。</span><div className="actions">{reviewShots.map(item => <button key={item.id} onClick={() => selectReviewShot(item.id)}>查看第 {item.index + 1} 镜 · {item.title}</button>)}</div></div>}<a className="primary" href={assetUrl(episode.final_video_asset_id, true)} download><Download size={16} />下载成片</a></div><video src={assetUrl(episode.final_video_asset_id)} controls preload="metadata" aria-label="最终短片" /></section>}
    {reviewing ? episode.status === 'AWAITING_REVIEW' ? <><EpisodePreview key={id} episode={episode} busy={busy} setBusy={setBusy} refresh={() => { resource.refresh(); jobs.refresh(); }} notify={notify} /><button className="text-button" disabled={busy} onClick={() => void action(`/episodes/${id}/preview`, '正在更新分镜预览')}>工作流配置有变化？更新分镜预览</button></> : <Empty title={active ? '导演正在准备分镜和提示词' : '继续准备分镜即可预览'}>预览准备完成后会等待你确认，再开始图片与视频生成。</Empty> : !episode.shots.length ? <Empty title={active ? '导演正在规划故事' : '你的故事，即将成为镜头'}>计划完成后，镜头、首尾帧和视频将在这里出现。</Empty> : <div className="editing-grid"><aside className="panel shot-list"><div className="panel-title">镜头时间线 <span>{episode.shots.length} SHOTS</span></div>{episode.shots.map((item, index) => <div key={item.id} className={`shot-row ${shot?.id === item.id ? 'selected' : ''} ${!item.enabled ? 'disabled' : ''}`} draggable={!active && !busy && !recovering && !disconnected} onDragStart={() => setDragged(item.id)} onDragOver={event => event.preventDefault()} onDrop={() => { const from = episode.shots.findIndex(item => item.id === dragged); if (from >= 0) move(from, index); setDragged(undefined); }}><button className="shot-select" onClick={() => setSelected(item.id)}><span className="shot-number">{String(index + 1).padStart(2, '0')}</span><div><strong>{item.title}</strong><small>{item.duration.toFixed(2)}s · {item.transition_from_previous}</small><Badge status={item.status === 'PASSED' && item.needs_review ? 'NEEDS_REVIEW' : item.status} /></div><GripVertical size={14} /></button><div className="shot-controls"><label><input type="checkbox" checked={item.enabled} disabled={active || busy || recovering || disconnected} onChange={event => void updateTimeline(episode.shots.map(s => s.id === item.id ? { ...s, enabled: event.target.checked } : s))} />使用</label><button className="icon-button" aria-label={`上移镜头 ${index + 1}`} disabled={active || busy || recovering || disconnected || index === 0} onClick={() => move(index, index - 1)}><ArrowUp size={14} /></button><button className="icon-button" aria-label={`下移镜头 ${index + 1}`} disabled={active || busy || recovering || disconnected || index === episode.shots.length - 1} onClick={() => move(index, index + 1)}><ArrowDown size={14} /></button></div></div>)}</aside>
      {shot && <section className="shot-detail" id="episode-shot-detail"><div className="panel"><div className="panel-title">{shot.title}<Badge status={shot.status === 'PASSED' && shot.needs_review ? 'NEEDS_REVIEW' : shot.status} /></div><p className="shot-description">{shot.action}</p>{shot.needs_review && <div className="notice shot-review" role="status"><strong>画面待复核</strong><span>生成结果已保留，可先看镜头视频，再决定是否重跑。</span>{(shot.review_notes ?? []).map((note, index) => <p key={index}><strong>{qaStageLabel(note.stage)}</strong> · {note.message}</p>)}</div>}<small className="muted">{shot.camera}</small><div className="keyframe-grid"><Media id={shot.start_frame_asset_id} kind="image" label="首帧 / START" />{shot.end_frame_asset_id && <Media id={shot.end_frame_asset_id} kind="image" label="尾帧 / END" />}</div><Media id={shot.video_asset_id} kind="video" label="镜头预览 / CLIP" />{shot.error && <ErrorNotice>{shot.error.message}</ErrorNotice>}</div>
