...
2. 打开“工作流”：选择内置模板或上传 **API Format JSON**；普通 UI JSON 需要先在 ComfyUI 导出 API 格式。系统自动识别常见工作流用途，无法确定时再选择生成方式。
3. 在“输入与输出”仅确认待定项目，例如多个保存节点中的最终输出。点击“保存并检查依赖”，按提示准备模型或节点；然后填写测试描述、上传对应素材并试跑，通过后设置默认项。首帧视频只需起始画面，首尾帧视频还需结束画面。
...
“高级设置”保留全部手动绑定、参数归属、模型参数及帧数转换；“自动补齐未绑定项”只补充可确定的缺项，不覆盖已有选择。未知模型的帧数规则需要手动确认 FPS、倍数和偏移。旧记录读取时不会被自动改写；导入时选错媒体类型的记录需按正确用途重新导入。
...
导入时识别文生图、图生图、首尾帧生成视频或首帧生成视频，可手动调整识别用途。图生图必须绑定 reference_image，视频绑定 start_frame/duration；首尾帧视频还需 end_frame。初始参考资产必须有可运行的文生图工作流。内置模板仍只有 SD1.5 文生图和 Wan 首尾帧；其他能力通过导入适配，不代表已安装相应模型。
