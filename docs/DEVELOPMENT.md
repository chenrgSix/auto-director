...
对结果不满意，展开「重跑任务 → 选择重跑范围」，选择单镜或全部启用镜头，以及仅视频或关键帧及视频。确认后保存旧版本并开始生成，剧本/提示词/参考图保持。连续镜头依赖前镜尾帧时会一并更新，页面显示影响数量。默认尝试新种子；高级模式明确固定的种子优先。下方「重跑历史」可预览旧图/视频、下载旧成片。重跑不会删除原媒体文件。
