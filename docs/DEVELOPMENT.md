...
保存后可点击“测试导演模型”执行最小 JSON 请求；“测试视觉模型”会发送一张内置小图验证图像输入。结果显示实际模型、耗时和具体错误，最多等待 120 秒，可取消；修改配置会清除旧结果。测试不会创建短片或渲染任务，小请求成功不保证长任务稳定。运行参数中的请求超时仅用于 ComfyUI，模型请求仍采用独立的 120 秒限制。
