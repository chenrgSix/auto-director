...
`agents.schemas.VisualContinuity` 与普通 Shot/批次 schema、创作包共用。`generation.continuity` 仅检查创作意图，按稳定场景维护状态并按实际工作流输入选择参考；它不能判断真实像素是否符合目标。新直接制作也先准备全部提示词再提交参考图，避免到第二角色才发现选择含糊。首帧来源从实际选中的 job 记录，保留用户覆盖和多候选选择结果。

`generation.continuity_review` 提供带标签的边界/目标采样和外部结论。内容 key 包含当前及前镜素材、目标、启用状态与参考选择；写入使用 Store 原子事务、版本检查和请求回执。HTTP 与 MCP 共用服务，MCP 无 Codex 会话占用，无额外模型调用。网页预览可补充结构化约束，详情页可看边界并记录观察。外部视觉通过只作用于当前素材的展示，不把旧模型 QA 记录删除或改为通过。
