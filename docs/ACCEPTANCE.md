...
- 实际短片 c24acc8b 的全部 6 镜按当前 MiniMax 元数据预检通过，13 类节点依赖有效，尺寸 384×672，length=124、FPS=24。仅查询所用节点元数据，零 POST /prompt；Episode version=51 完整记录未改动。预检和原记录备份位于忽略目录 data/repairs/c25-dimensions/。实际服务及缓存预算更新随本轮警告收尾执行，未进行新真实模型画质验收。
...
- 正式 8000 正在处理另一部短片的真实生成，因此没有重启；修复代码需在下次服务重启后加载。本轮未新增真实渲染、未执行远端 CI。
