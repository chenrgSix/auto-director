...
C08 增加显式更换入口：已创建短片可通过 `PATCH /episodes/{id}/workflows` 更新三类 ID，但必须停止并核对未完成作业。C18 起未渲染故事保留计划、镜头、时长及提示词，只校验新能力与参数；不兼容则保留原配置。已产生当前绑定的渲染作业/素材时才归档重置；自动选择仍不随全局默认变动。详见 [接口契约](API_CONTRACT.md#更换已创建短片的工作流c08)。
