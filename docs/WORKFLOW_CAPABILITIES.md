...
C08 增加显式更换入口：已创建短片可通过 `PATCH /episodes/{id}/workflows` 更新三类 ID，但必须停止并核对未完成作业。更换归档旧进度、重置计划与生成结果，再按新工作流能力重新规划；自动选择仍不随全局默认变动。详见 [接口契约](API_CONTRACT.md#更换已创建短片的工作流c08)。
