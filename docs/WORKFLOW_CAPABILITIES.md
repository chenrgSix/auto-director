# Workflow Capability 与参数归属（C03 / C04 / C05）
...
上传 API JSON 后先分析生成用途、可写输入和最终保存节点。常见节点沿条件与素材连线识别，无需先添加 Input/Output 标签；唯一且类型适配的候选自动绑定，歧义项目由用户确认。标签和已有手动绑定保留优先权，补齐操作只填缺项。未知模型帧数规则保留为高级确认项，不能套用默认帧数转换。

默认页面显示中文用途卡片、待确认候选、模型/节点依赖与独立试跑区域。I2V 隐藏结束画面绑定及试跑上传项；完整字段、owner、覆盖策略和时长转换折叠在高级设置。保存后同步服务端返回值，依赖检查成功不等于试跑成功。旧 profile 不自动改写，不新增数据库迁移。
