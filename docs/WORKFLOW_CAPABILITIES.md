...
C11：Shot.duration 表示时间线长度，视频作业另记 timeline_duration 和实际渲染 duration。秒数节点根据已选模型的类型、min/max/step/enum 向上取合法值（2.86 秒可生成 4 秒后裁切），不超出 workflow 与模型的时长上限；帧数节点保留既有转换。C20 起显存检测与质量模式不覆盖已配置的时长，API 元数据继续用于执行方式说明；图片继续遵守本地显存策略，实际 OOM 仍执行降分辨率/分段恢复。无合法时长时提前失败，不生成参考图。
